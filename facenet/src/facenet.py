...
def prewhiten(x,x_stack):
    #print('x_stack shape '+str(np.shape(x_stack)))
    mean = np.mean(x_stack,axis=0)
    std = np.std(x_stack,axis=0)
    std_adj = np.maximum(std, 1.0/np.sqrt(x.size))
    #y = np.multiply(np.subtract(x, mean), 1/std_adj)
    #print('means shape '+str(np.shape(mean)))
    #print('std_adj '+str(np.shape(std_adj)))
    y = (x-mean)/std_adj
    return y 
...
def load_data(image_paths, do_random_crop, do_random_flip, image_size, do_prewhiten=True):
    ...
    for i in range(nrof_samples):
        #if ".err" in image_paths[i]:
        #    continue
        img = misc.imread(image_paths[i])
        if img.ndim == 2:
            img = to_rgb(img)
        if np.shape(img)[2] is not 3:
            img = img[:,:,:3]
        img = crop(img, do_random_crop, image_size)
        img = flip(img, do_random_flip)
        img_list[i] = img
        #print(str(i)+"."+str(np.shape(img)))
    images = np.stack(img_list)
    #print(images)
    images_whiten = []
    if do_prewhiten:
        for img in images:
            img_whiten = prewhiten(img,images)
            #img_whiten = img
            images_whiten.append(img_whiten)
    #print("images_shape "+str(np.shape(images_whiten)))
    #print(img_whiten)
    return np.asarray(images_whiten)
...
def select_triplets(embeddings, num_per_class, image_data, people_per_epoch, alpha):

    print("len_embeddings: "+str(len(embeddings)))
    print("num_per_class: "+str(num_per_class))
    print("people_per_epoch: "+str(people_per_epoch))
    print("len_image_data: "+str(len(image_data)))
    def dist(emb1, emb2):
        x = np.square(np.subtract(emb1, emb2))
        return np.sum(x, 0)
  
    nrof_images = image_data.shape[0]
    nrof_triplets = nrof_images - people_per_epoch
    ...
    emb_start_idx = 0
    nrof_random_negs = 0
    for i in xrange(people_per_epoch):
        n = num_per_class[i] #It's sure that len(num_per_class) >= people_per_epoch
        for j in range(1,n):
            a_idx = emb_start_idx
            ...
            #sel_neg_idx := minn_mark(in my own name system)
            #sel_neg_idx := minn(on my own)
        ...
        emb_start_idx += n
    print("triplet_num: "+str(trip_idx))
...
def get_dataset(paths):
    ...
    for path in paths.split(':'):
        ...
        classes_with_txt = os.listdir(path_exp)
        classes = [d for d in classes_with_txt if ".txt" not in d]
        classes.sort()
        ...
        for i in range(nrof_classes):
            ...
            if os.path.isdir(facedir):
                ...
                image_paths = [os.path.join(facedir,img) for img in images if ".err" not in img]
                dataset.append(ImageClass(class_name, image_paths))
    return dataset

def sample_people(dataset, people_per_epoch, images_per_person):
    #!!!may cause problem!!!
    #nrof_images = people_per_epoch* images_per_person
    ...
    # Considering I don't have enough images, a recurrent method is applied in the next line. In fact, I find the method is useless because the number of people is strictly controlled by people_per_epoch in function select_triplets() 23333
    nrof_images = people_per_epoch*images_per_person
    # images_per_person is useless because I don't have so much images per person'
    #while len(image_paths)<nrof_images < nrof_classes:
    while i < people_per_epoch:
        class_index = class_indices[i]
        nrof_images_in_class = len(dataset[class_index])
        image_indices = np.arange(nrof_images_in_class)
        np.random.shuffle(image_indices)
        nrof_images_from_class = min(nrof_images_in_class, images_per_person, nrof_images-len(image_paths))
        print("num_in_class: "+str(nrof_images_from_class))
